...
# Colores utilizados en la interfaz y los gráficos
FONDO_FIGURA = "#eef3f7"
FONDO_EJES = "#ffffff"
COLOR_TEXTO = "#27374d"
...
# Paleta de colores para mapas de calor de similitud
CMAP_SIMILITUD = sns.color_palette(
    ["#ffffff", "#dde6ed", "#9db2bf", "#526d82", "#27374d"],
    as_cmap=True,
)

# Paleta de colores para la matriz documento-término
...
def aplicar_tema_grafico(fig, ax) -> None:
    """Aplica colores y estilo visual a un gráfico."""
    ...
    # Aplica color a los bordes del gráfico
    for spine in ax.spines.values():
        spine.set_color(COLOR_GRID)


def aplicar_tema_colorbar(ax) -> None:
    """Aplica formato visual a la barra de colores."""
...
def crear_grafico_barras_similitud(resultados: pd.DataFrame):
    """Genera un gráfico de barras con la similitud de cada documento."""
    ...
    ax.barh(
        datos["Documento"] + " - " + datos["Tema"],
        datos["Similitud coseno"],
        color=colores,
    )
    ...
    # Muestra el valor de similitud sobre cada barra
...
def crear_grafico_terminos(matriz_df: pd.DataFrame, tipo_vectorizador: str):
    ...
    etiqueta_eje = (
        ...
        else "Frecuencia total"
    )

    fig, ax = plt.subplots(figsize=(10, 4.8))

    puntajes.sort_values(ascending=True).plot(
        kind="barh",
        color=COLOR_VERDE,
        ax=ax,
    )
